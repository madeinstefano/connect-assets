...
Assets.prototype.serveAsset = function (req, res, next) {
  ...
  if (!asset.buffer) {
    ...
    if ( this.options.gzip && !this.manifest.assets[parts.path] ) {
      var deflated = zlib.gzipSync( Buffer.from( asset.buffer ).toString() );
      res.setHeader( "Content-Length", deflated.length );
